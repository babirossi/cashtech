...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/appfinance'
...
@app.route('/somar_receitas')
def somar_receitas():
    receitas = Receitas.query.all()  # Supondo que Receita seja o modelo de dados das receitas
    total_receitas = sum(receita.valor for receita in receitas)
    return f'O valor total das receitas é: {total_receitas}'
